...
1. Apps Script エディタで、デフォルトの `コード.gs` ファイルをクリック
2. 既存のコードを削除し、`Code.gs` の内容をすべてコピー＆ペースト

#### Sidebar.html の設定

1. `ファイル` メニュー → `新規` → `HTML ファイル` を選択
2. ファイル名を `Sidebar` に設定
3. `Sidebar.html` の内容をすべてコピー＆ペースト
