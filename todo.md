...
- [x] 🚨 **ユーザープロンプト完全保持システム実装**
  - **重大改善**: ユーザープロンプトは絶対に省略しない方針に変更
  - **省略対象の限定**: GPT-Image-1 の内部処理版（revised_prompt）のみを省略対象に
  - **ユーザーデータ完全保護**: セルコメント、C 列、B 列でユーザープロンプト完全保持
  - **統計情報簡潔化**: 装飾文字を削減してスペース確保（📊→ 変更度、🤖GPT-Image-1 内部処理版 → 内部処理版）
  - **50,000 文字制限対策**: 内部処理版のみ制限でエラー完全根絶 + ユーザーデータ完全保護
