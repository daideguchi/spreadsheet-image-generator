...
- [x] 🎯 DALL-E 3 を Web 版完全準拠に修正（2024 年 12 月最終版）
  - quality を "standard" から "hd" に変更（Web 版 ChatGPT は常に HD 使用）
  - style を常に "natural" に固定（Web 版デフォルト）
  - プロンプト改変防止指示を追加（Web 版互換性）
  - 複雑なスタイル判定ロジックを完全削除
  - サイズ判定のみの最小限解析に変更
  - Web 版 ChatGPT の DALL-E 3 と完全に同じパラメータ設定
  - プロンプト忠実性の大幅向上を実現
