...
- [x] 🔴 画像削除機能の完全実装
  ...
  - H 列チェックボックスによる選択画像削除機能を新規実装
  - 全画像一括削除機能（確認ダイアログ付き）を追加
  - 画像のみ削除・他データ完全保持システムを構築
  - 削除後のステータス自動更新（🗑️ 画像削除 → 🔄 復元可能）
  - 削除後の行高最適化・復元準備機能を実装
  - サイドバーに削除ボタン（選択削除・全削除）を追加
  - 削除機能専用の danger ボタンスタイルを適用
  - プロンプト保持による再生成可能性を確保
    - エラーハンドリング・ユーザビリティを最大化

- [x] 🔴 画像生成プログレスモーダルの最適化

  - ステップ 5 の異常な長時間表示を修正（60%→5%に調整）
  - 実際の画像生成枚数をリアルタイム表示に対応
  - 各ステップの時間配分を実際の処理時間に合わせて最適化
  - getImageGenerationCount()関数で未生成画像数を正確に取得
  - getSelectedImageCount()関数で選択画像数を事前取得
  - 画像枚数に応じた動的な進捗時間調整システムを実装
  - 「プロンプトを解析中... (3 枚予定)」形式で枚数を明示
  - 「画像を生成中... (3 枚)」で実際の生成枚数を表示
  - 完了メッセージを「✅ 画像生成が完了しました！」に改善
    - より自然で正確な進捗表示でユーザー体験を向上

- [x] 🔴 OpenAI API 500 エラー対応・堅牢性向上
  - 重大な 500 サーバーエラーに対する完全なリトライ機能を実装
  - muteHttpExceptions 追加で詳細なエラー情報取得を実現
  - 最大 3 回のリトライ（2 秒、4 秒、6 秒の段階的待機）
  - レート制限（429 エラー）対応で 60 秒待機機能
  - 部分成功システム（一部失敗でも他の画像は生成継続）
  - 失敗画像の視覚的表示（赤背景・エラーメッセージ）
  - 個別画像エラーハンドリングで全体停止を回避
  - 詳細なエラーログ・成功/失敗カウント表示
  - 失敗画像の再生成ガイダンス機能
  - OpenAI API の不安定性に対する完全な耐性を構築
