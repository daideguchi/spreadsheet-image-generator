...
function createCommonPromptSheet() {
  try {
    ...
    // ヘッダー設定
    const headers = ["カテゴリ", "プロンプト名", "プロンプト内容", "説明"];
    ...
    // 列幅設定
    commonSheet.setColumnWidth(1, 120); // カテゴリ
    commonSheet.setColumnWidth(2, 150); // プロンプト名
    commonSheet.setColumnWidth(3, 400); // プロンプト内容
    commonSheet.setColumnWidth(4, 200); // 説明
    ...
    const defaultPrompts = [
      ...
      [
        "🎨 スタイル",
        "油絵風",
        "oil painting style, classical art, brushstrokes",
        "伝統的な油絵のタッチ",
      ],
      [
        "🎨 スタイル",
      ...
      [
        "🌅 環境",
        "夕日背景",
        "beautiful sunset, golden hour lighting, warm colors",
        "美しい夕日の背景",
      ],
      [
        "🌅 環境",
        "森林背景",
        "lush forest, natural environment, green trees",
        "豊かな森林の背景",
      ],
      [
        "🌅 環境",
        "都市背景",
        "modern city, urban environment, skyscrapers",
        "現代的な都市の背景",
      ],
      [
        "🌅 環境",
        "海辺背景",
        "ocean view, beach, blue water, waves",
        "美しい海辺の背景",
      ],
      [
        "📸 品質",
        "高品質",
        "4K, ultra high resolution, masterpiece, best quality",
        "最高品質の画像生成",
      ],
      [
        "📸 品質",
        "芸術的",
        "artistic composition, creative, unique perspective",
        "芸術的で創造的な構図",
      ],
      [
        "🎭 ムード",
        "明るい",
        "bright, cheerful, positive atmosphere, vibrant",
        "明るく前向きな雰囲気",
      ],
      [
        "🎭 ムード",
        "落ち着いた",
        "calm, peaceful, serene, tranquil atmosphere",
        "静かで落ち着いた雰囲気",
      ],
      [
        "🎭 ムード",
        "神秘的",
        "mysterious, magical, ethereal, dreamlike",
        "神秘的で幻想的な雰囲気",
      ],
      [
        "🎭 ムード",
        "ドラマチック",
        "dramatic lighting, intense, powerful atmosphere",
        "ドラマチックで力強い雰囲気",
      ],
    ];
    ...
    for (let i = 2; i <= defaultPrompts.length + 1; i++) {
      ...
      if (category.includes("🎨")) {
        rowRange.setBackground("#fff3e0"); // オレンジ系
      } else if (category.includes("🌅")) {
        rowRange.setBackground("#e8f5e8"); // 緑系
      } else if (category.includes("📸")) {
        rowRange.setBackground("#e3f2fd"); // 青系
      } else if (category.includes("🎭")) {
        rowRange.setBackground("#f3e5f5"); // 紫系
      }
    }

    // 使用説明を追加
    const instructionRow = defaultPrompts.length + 3;
    const instructionRange = commonSheet.getRange(instructionRow, 1, 1, 4);
    instructionRange.merge();
    instructionRange.setValue(
      "💡 使用方法：新しい共通プロンプトを追加するには、上記の表に新しい行を追加してください。メインシートのドロップダウンに自動反映されます。"
    );
    instructionRange.setBackground("#fff9c4");
    instructionRange.setFontWeight("bold");
    instructionRange.setWrap(true);
    commonSheet.setRowHeight(instructionRow, 60);

    console.log("共通プロンプト設定シートを作成しました");
    return true;
  } catch (error) {
